...
export function ReportPage({ onNavigate, data }: ReportPageProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const fullShareUrl = `${window.location.origin}/dashboard?reportId=${shareId}`;
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'contrast': return <Accessibility className="w-3 h-3" />;
      case 'heuristic': return <Eye className="w-3 h-3" />;
      case 'visual': return <Palette className="w-3 h-3" />;
      case 'business': return <Megaphone className="w-3 h-3" />;
  ...
  const getCategoryBadgeStyles = (category: string) => {
    switch (category) {
      ...
      case 'heuristic': return 'bg-blue-100 text-blue-700 border-blue-200';
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 overflow-x-hidden pb-12">
      ...
      <div className="max-w-4xl mx-auto px-6 py-12 space-y-10">
        ...
        {/* REPORT CARD — Single flowing document */}
        <div className="bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden report-card">
          ...
          {/* Visual Diagnostic — Screenshot with Heatmap */}
          <div className="px-10">
            <div className="screenshot-container relative rounded-2xl overflow-hidden border border-slate-200 shadow-inner bg-slate-950 flex justify-center items-center" ref={containerRef}>
              <div className="relative inline-block w-full">
                ...
                {dimensions.width > 0 && (
                  <HeatmapCanvas
                    ...
                    className="heatmap-overlay absolute inset-0 pointer-events-none mix-blend-screen opacity-90"
                  />
                )}
          ...
          {data.influencerReview && (
            <div className="px-10 pt-10">
              <div className="border-t border-slate-100 pt-10">
                ...
                <div className="space-y-8 bg-slate-50/50 p-8 rounded-2xl border border-slate-100">
          ...
          {/* Detailed Findings & Suggestions — OBSERVATION LOG */}
          <div className="px-10 pt-10">
            <div className="border-t border-slate-100 pt-10">
              <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-3">
              ...
              <div className="space-y-6">
                {annotations.map((ann) => (
                  <div key={ann.id} className="p-6 bg-slate-50 border border-slate-100 rounded-2xl flex gap-6 transition-all hover:bg-slate-100/50">
                    <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 shadow-sm ${ann.severity === 'critical' ? 'bg-red-500' : 'bg-blue-500'} text-white`}>
                      <span className="text-sm font-black">{ann.id}</span>
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-3 mb-2 flex-wrap">
                        <h4 className="text-base font-bold text-slate-900 leading-tight">{ann.title}</h4>
                        <Badge className={`text-[8px] h-4 uppercase font-black px-1.5 tracking-tighter border ${getCategoryBadgeStyles(ann.category)}`}>
                          {getCategoryIcon(ann.category)}
                          <span className="ml-1">{getCategoryLabel(ann.category)}</span>
                        </Badge>
                        {ann.tag && (
          ...
          {/* Footer */}
          <div className="px-10 py-8 mt-8 border-t border-slate-100 flex justify-between items-center text-[9px] text-slate-400 font-bold uppercase tracking-[0.2em]">
            <div>Design Snapper Pro • Ver 2.5.0</div>
